...
async def start(client:Client, message):
    ...
    if len(message.command) == 2 and message.command[1].startswith("reff_"):
        try:
            ...
            await message.reply_text("Iɴᴠᴀʟɪᴅ ʀᴇғᴇʀ ")
            return
        if user_id == message.from_user.id:
            await message.reply_text("Hᴇʏ ᴅᴜᴅᴇ, ʏᴏᴜ ᴄᴀɴ ɴᴏᴛ ʀᴇғᴇʀ ʏᴏᴜʀsᴇʟғ ")
            return
        ...
        try: 
            ...
            if refData and refData.split("-", 1)[0] == "Jisshu":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAJ-8GgKYEJwdlFBDJ4Kijv83Zi-Fl9xAAIrEwACBTWRV_obEOngC_wFHgQ") 
    ...
    if data and data.startswith("allfiles"):
        ...
        for file in files:
            ...
            btn = [[
                InlineKeyboardButton("Join", url=f'https://t.me/allmoviezonez')
            ]]
        ...
        try:
            ...
            for target_id in target_ids:
                try:
                    ...
                    out += f"  ᴇʀʀᴏʀ ɪɴ ᴛʜɪꜱ ɪᴅ - <code>{target_id}</code> <code>{str(e)}</code>\n"
...
async def all_settings(client, message):
    ...
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    settings = await get_settings(grp_id)
    text = f"""<b><u>  ʏᴏᴜʀ sᴇᴛᴛɪɴɢs ꜰᴏʀ -</u> {title}
